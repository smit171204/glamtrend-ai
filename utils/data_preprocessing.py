...
class MissingValueHandler:
    ...
    def smart_impute(self):
        ...
        for col in summary.index:
            ...
            # ✅ FIXED: proper dtype check
            if pd.api.types.is_numeric_dtype(self.df[col]):
                simple_value = self.df[col].median()
            else:
                simple_value = self.df[col].mode()[0]

            # -------------------------
            # LOW MISSING → SIMPLE
            # -------------------------
            if missing_percent < 5:
                self.df[col].fillna(simple_value, inplace=True)
